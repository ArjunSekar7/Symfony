...
class InstrumentHistory
{
    ...
    function connect()
    {
        $con = mysqli_connect("localhost", "root", "password", "ShareMarket");
        return $con;
    }
    ...
    {
        $id        = $this->getId($historyFile);
        $value     = (float) $historyData['value'];
        $con       = $this->connect();
        $statement = $con->prepare("insert into mv_instrument_ohlc_eod values($id,'$historyData[date]', $value, $value, $value, $value)");
